...
def test_sanity_ivhd(device, optimizer):
    ...
    X = torch.zeros((6, 3))
    ivhd = IVHD(2, 2, 1, 0.3, optimizer=optimizer, lr=0.1, epochs=300, eta=1., device=device)
...
def test_mnist_part_ivhd(device, optimizer, N, NN, RN):
    ...
    ivhd = IVHD(2, NN, RN, 0.4, optimizer=optimizer, lr=0.08, epochs=600, eta=1., device=device)
