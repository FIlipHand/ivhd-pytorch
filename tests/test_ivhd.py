...
def test_sanity_ivhd(device, optimizer):
    ...
    X = torch.zeros((6, 3))
    ivhd = IVHD(2, 2, 1, 0.1, optimizer=optimizer, lr=2., epochs=500, eta=1., device=device)
...
@pytest.mark.parametrize("device", ["cpu", "cuda:0"])
@pytest.mark.parametrize("optimizer", [None, torch.optim.Adam, torch.optim.SGD, torch.optim.Adagrad])
@pytest.mark.parametrize("N", [100, 10])
...
def test_mnist_part_ivhd(device, optimizer, N, NN, RN):
    ...
    ivhd = IVHD(2, NN, RN, 0.5, optimizer=optimizer, lr=0.01, epochs=500, eta=1., device=device)
